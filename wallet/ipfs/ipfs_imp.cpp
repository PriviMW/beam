        ...
        //
        {
            ...
            asio::spawn(_ios.get_executor(), [&](boost::asio::yield_context yield) {
                try
                {
                    ...
                    // cannot work on Boost 1.90: build_() stores the completion handler in
                    // a std::function (CopyConstructible), but Boost 1.90's yield completion
                    // handler for a unique_ptr result is move-only (deleted copy ctor). The
                    // add/cat/calc_cid Android bypasses already avoid asio coroutines; do the
                    // same for node construction via the synchronous node() constructor
                    // (go_asio_ipfs_start_blocking). The lifetime _ios_guard + dedicated
                    // _thread below keep _ios alive for ongoing ops, so this only affects
                    // the initial build step.
                    (void)yield;
                    _node = std::make_unique<asio_ipfs::node>(_ios, scb, config);
#else
