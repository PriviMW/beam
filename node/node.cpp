	...
	for (TaskSet::iterator it = m_setTasks.begin(); m_setTasks.end() != it; ++it)
	{
		...
		else
		{
			...
			if (t.m_sidTrg.m_Number.v > t.m_Key.first.m_Number.v)
				std::setmax(hDoneHdrs, m_Processor.m_Cursor.m_Full.m_Number.v + t.m_sidTrg.m_Number.v - t.m_Key.first.m_Number.v);
		}
	}
...
void Node::DeleteOutdated()
{
	...
	auto hTop = m_Processor.m_Cursor.m_hh.m_Height;
...
void Node::MaybeGenerateRecovery()
{
	...
	Height h1 = m_Processor.m_Cursor.m_hh.m_Height;
...
void Node::Processor::OnRolledBack()
{
	...
	while (!txp.m_lstOutdated.empty())
	{
		...
		if (x.m_Hist.m_Height <= m_Cursor.m_hh.m_Height)
			break;
...
{
	return m_This.m_Processor.m_Cursor.m_hh.m_Height + 1;
}
...
{
	ctx.m_Height.m_Min = m_Processor.m_Cursor.m_hh.m_Height + 1;
...
uint8_t Node::OnTransactionStem(Transaction::Ptr&& ptx, std::ostream* pExtraInfo)
{
	...
	if (pF)
	{
		...
		if (pF->m_Hist.m_Height == m_Processor.m_Cursor.m_hh.m_Height)
		{
	...
	if (!pF)
	{
		...
		pF = m_TxPool.AddValidTx(std::move(pTxOrig), stats, keyTx, TxPool::Fluff::State::PreFluffed, m_Processor.m_Cursor.m_hh.m_Height);
		m_Wtx.Delete(keyTx);
	}
...
void Node::AddDummyInputs(Transaction& tx, TxPool::Stats& stats)
{
	...
	while (tx.m_vInputs.size() < m_Cfg.m_Dandelion.m_OutputsMax)
	{
		...
		if (h > m_Processor.m_Cursor.m_hh.m_Height)
			break;
...
void Node::AddDummyOutputs(Transaction& tx, TxPool::Stats& stats)
{
	...
	// add dummy outputs
	bool bModified = false;
	auto& fs = Transaction::FeeSettings::get(m_Processor.m_Cursor.m_hh.m_Height + 1);
	...
	while (tx.m_vOutputs.size() < m_Cfg.m_Dandelion.m_OutputsMin)
	{
		...
		ECC::Scalar::Native sk;
		pOutput->Create(m_Processor.m_Cursor.m_hh.m_Height + 1, sk, *m_Keys.m_pMiner, cid, *m_Keys.m_pOwner);
...
Height Node::SampleDummySpentHeight()
{
	...
	Height h = m_Processor.m_Cursor.m_hh.m_Height + d.m_DummyLifetimeLo + 1;
...
uint8_t Node::OnTransactionFluff(Transaction::Ptr&& ptxArg, std::ostream* pExtraInfo, const PeerID* pSender, const TxPool::Stats* pStats)
{
	...
	if (!pStats)
	{
		bool bTested = pF && (pF->m_Hist.m_Height == m_Processor.m_Cursor.m_hh.m_Height);
		if (!bTested)
		{
...
{
	if (Rules::get().IsPastFork_<1>(m_This.m_Processor.m_Cursor.m_hh.m_Height) && (Rules::Consensus::FakePoW != Rules::get().m_Consensus))
	{
		// test the hash
		ECC::Hash::Value hv;
...
{
	Processor& p = m_This.m_Processor;
	Height h = std::min(msg.m_Height, p.m_Cursor.m_hh.m_Height); // don't throw exc if the height is too high, theoretically height can go down due to reorg (of course the chainwork must be higher anyway)
...
void Node::Peer::OnMsg(proto::GetAssetsListAt&& msg)
{
	...
	bool bCurrent = (msg.m_Height >= m_This.m_Processor.m_Cursor.m_hh.m_Height);
...
bool Node::GenerateRecoveryInfo(const char* szPath)
{
	...
	try
	{
		ctx.m_Writer.Open(szPath, m_Processor.m_Cwp, m_Processor.m_Cursor.m_hh.m_Height);
		m_Processor.get_Utxos().Traverse(ctx);

		const Rules& r = Rules::get();

		if (r.IsPastFork_<2>(m_Processor.m_Cursor.m_hh.m_Height))
		{
			...
			{
				if (!r.IsPastFork_<6>(m_Processor.m_Cursor.m_hh.m_Height))
					ai.SetCid(nullptr);
			...
			if (r.IsPastFork_<3>(m_Processor.m_Cursor.m_hh.m_Height))
			{
...
void Node::Validator::ShuffleRoundData(bool bConsequent)
{
	auto& rdcurr = m_pR[0];
	auto& rdnext = m_pR[1];

	if (bConsequent)
		rdcurr = std::move(rdnext);
	else
		rdcurr.Reset();
...
Node::Validator::ValidatorPlus* Node::Validator::ValidatorSet::Find(const Block::Pbft::Address& addr) const
{
	auto it = m_mapValidators.find(addr, ValidatorPlus::Comparator());
...
void Node::Validator::Power::Add(const Power& pwr)
{
	m_wVoted += pwr.m_wVoted;
	m_nWhite += pwr.m_nWhite;
}
...
void Node::Validator::Vote(uint8_t iKind)
{
	...
	if (m_pMe->m_pR[iR].m_pVote[iKind])
	...
	Sign(msg.m_Signature, rd.m_pPwr[iKind].m_hv);

	m_pMe->m_pR[iR].m_pVote[iKind] = msg.m_Signature;
	rd.m_pPwr[iKind].Add(*m_pMe);
...
void Node::Validator::OnMsg(proto::PbftProposal&& msg, const Peer& src)
{
	...
	auto tm_ms = rd.m_Proposal.m_Msg.m_Hdr.get_Timestamp_ms();
	RoundTimes rt;
	rt.m_t0 = tm_ms;
...
void Node::Validator::MakeFullHdr(Block::SystemState::Full& s, const Block::SystemState::Sequence::Element& el) const
{
	auto& p = get_ParentObj().m_Processor; // alias
	...
	{
		s.m_Prev = p.m_Cursor.m_hh.m_Hash;
